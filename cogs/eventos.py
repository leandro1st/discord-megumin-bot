...
import tweepy
from cogs.outros import auth, api, sauce, sauce2
...
class Eventos(commands.Cog, name="Eventos"):
    ...
    async def on_member_join(self, member):
        channel = discord.utils.get(
            member.guild.text_channels, name="bem_vindo")
    ...
    async def search_sauce(self, message, url):
        ...
        # Similarity
        embed.add_field(name="Similarity".format(),
                        value="{}%".format(best.similarity), inline=True)

        # Author
        if best.author:
            embed.add_field(name="Author".format(),
                            value="{}".format(best.author), inline=True)
        else:
        ...
        if 'material' in best.raw['data']:
            if best.raw['data']['material']:
                embed.add_field(name="Material", value="{}".format(
                    best.raw['data']['material']), inline=False)

        # Characters
        if 'characters' in best.raw['data']:
            if best.raw['data']['characters']:
                embed.add_field(name="Characters", value="{}".format(
                    best.raw['data']['characters']), inline=False)
        ...
        if isinstance(best, VideoSauce):
            ...
            if 'part' in best.raw['data']:
                if best.raw['data']['part']:
                    embed.add_field(name="Episode", value="{}".format(
                        best.raw['data']['part']), inline=False)
            # Year
            if 'year' in best.raw['data']:
                if best.raw['data']['year']:
                    embed.add_field(name="Year", value="{}".format(
                        best.raw['data']['year']), inline=False)
            # Estimated time
            if 'est_time' in best.raw['data']:
                if best.raw['data']['est_time']:
                    embed.add_field(name="Est Time", value="{}".format(
                        best.raw['data']['est_time']), inline=False)
        # If source is from a book
        elif isinstance(best, BookSauce):
            # Part/volume
            if 'part' in best.raw['data']:
                if best.raw['data']['part']:
                    embed.add_field(name="Part", value="{}".format(
                        best.raw['data']['part']), inline=False)
        ...
        if 'source' in best.raw['data']:
            if best.raw['data']['source']:
                ...
                if re.compile('^https:\/\/i\.pximg\.net').search(best.raw['data']['source']):
                    embed.add_field(name="Source", value='https://www.pixiv.net/en/artworks/' +
                                    best.raw['data']['source'].split('/')[-1], inline=False)
                # Other links
                else:
                    embed.add_field(
                        name="Source", value=best.raw['data']['source'], inline=False)

        # Links
        for count, link in enumerate(best.urls, start=1):
            embed.add_field(name="Link {}".format(count),
                            value="{}".format(link), inline=False)
    ...
    async def change_status(self):
        ...
        status1 = ["!help", "with lolis", "Terraria", "osu!", "Visual Studio Code", "Nekopara", "Genshin Impact",
                   "Princess Connect! Re: Dive", "Muse Dash", "Geometry Dash", "Little Witch Nobeta", "Euro Truck Simulator 2", "Apex Legends"]
